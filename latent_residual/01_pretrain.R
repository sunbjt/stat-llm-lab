...
luz_callback_simple_batch <- luz_callback(
  "simple_batch",
  initialize = function(
    filename = paste0("checkpoints/lrp_loss_", format(Sys.time(), "%d%H%M"), ".csv")
    ) {
    self$file <- filename
  },
...
luz_callback_simple_batch <- luz_callback(
  ...
  initialize = function(
    filename = paste0("checkpoints/lrp_pre_loss_", format(Sys.time(), "%H%M%S"), ".csv")
    ) {
    self$file <- filename
  },
...
stage2_callbacks <- list(
  luz_callback_warm_start("checkpoints/lrp_02.pt", base_lr = 1e-4),
  ...
  luz_callback_model_checkpoint(
    path = "checkpoints/stage2_lrp_{epoch:02d}.pt",
