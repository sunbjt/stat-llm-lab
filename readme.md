...
软蒸最好的办法是 tokenizer 一致，这样 token 的概率可以一一对应。到我们这个项目，小模型的词表大小为 2^14，远低于 Qwen/gemma 等模型的词表。
这里应对的策略是做词表的对应归一。
...
本项目使用的是利用的依然还是 800 兆 JSONL 原始数据进行的蒸馏训练，该过程涉及教师模型推理（Qwen3.5-0.8B）。
考虑到生态的完整性，使用 Python 将软标签数据以 arrow 的格式落盘，整个原始数据推理时间约为 5 小时。
...
读者也可以考虑完全无预训练文本，而是通过对教师模型下达合成指令的方式（无显式的预训练数据）来训练模型。
