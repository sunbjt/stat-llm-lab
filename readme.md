...
本项目使用 R 语言的 `torch` + `luz` 深度学习框架，实现了多种 LLM（大语言模型）架构，涵盖从经典因果语言模型到实验性的 JEPA、对比学习、潜在空间残差预测、MoE 模型训练等前沿方法，以及蒸馏（软标签）、UI 部署、大模型的评测、统计水印（Deepfake 披露）等技术。

读者有兴趣可以访问在线 Demo（潜在空间残差预测）：<http://110.40.168.251:8080>

<div align="center">
<img src="img/server.png" alt="服务示例" width="85%">
</div>

五类模型在笔者自行构建的 1300 道测试题上的准确率介于 27.92% 至 30.46% 之间：

- MoE 架构的 Causal LM (30.46%) 与 对比学习 (Contrastive) (30.23%) 处于第一梯队，突破了 30% 大关。
- Causal LM (29.54%) 表现稳健，作为 Baseline 依然打扎实。
- Latent Residual Predictor (28.31%) 和 JEPA VQ (27.92%) 稍显落后。

26 个测评类评估效果见下：

<div align="center">
<img src="img/acc.png" alt="评估效果" width="85%">
</div>

由于各个模型的架构设计不同，导致在评估时，不同模型在不同任务上的准确率表现也不同。
...
为了让模型有一定智能，数据经过严格的挑选和处理，具体步骤可以参考第四节。实际预训练语料采用标准 JSONL 格式，0.8G 大小，71 万行，经过 BPE 分词后约 180M token。按照 Chinchilla 定律，15M 参数规模的模型应该为之配备 300M token 数据才能获得较好的训练效果，因此每个架构都重复训练了 2 个 epoch。

在 NVIDIA 3090 24G 服务器上，约 25 分钟左右一个 epoch，加之微调和 BPE 分词两个环节，约 1 小时即可完整复现 LLM。
...
最基础的语言模型实现，等价于一个小型 GPT。使用 `luz` 训练框架。
...
> **实验效果**：残差预测架构在损失曲线优于标准 `causal_lm`。其核心优势在于，相比标准 Transformer 将下一时刻的预测完全隐式地编码在 $H_t$ 中，LRP 通过显式的 $\Delta H$ 预测分支，在连续语义空间中建模 token 间的"语义偏移量"。这种归纳偏置使模型更有效地利用隐层表示，收敛更稳定，生成文本的局部连贯性也更佳。经对照实验验证，这一优势来自架构本身的残差预测设计，而非 zero-init 初始化技巧。
...
> **实验效果**：单纯 InfoNCE 损失收敛很慢，如果读者有兴趣可以参考 `pure_infonce` 文件夹脚本，即便在预训练多 1 个 epoch，最终的评估 acc 分数仅为 27%，推理环节肉眼可见的差。在 InfoNCE 基础上增加 20% 的 Cross Entropy 辅助是训练稳定的关键，效果优于纯粹的 InfoNCE。不过，两路推理路径中，交叉熵路径生成质量依然优于余弦相似度匹配路径。该实验验证了混合 loss 体系的复合架构确实是有效的落地路径。
...
该项目为了保证训练时间的可控，同时提高模型在某些领域智力水平，采用了三组数据采集策略：

1. 专业知识：爬取 wikipedia 上关于 `数据挖掘、深度学习、数据科学` 词条下的所有概念构建关系图，将 PageRank 值前 30000 的词条放入 Qwen3.7 flash 内构造合成语料。参考 `tidydata` 目录下 `08_crawler.r` 和 `08_translate_concept.R`脚本。
2. 通识知识：同样的逻辑，爬取 `China` 和 `Outline of academic disciplines` 所有词条构造合成语料。
3. 其他语料：如来自 [Minimind](https://github.com/jingyaogong/minimind) 项目（small 数据集）、 [Fineweb-Edu-Chinese](https://huggingface.co/datasets/opencsg/Fineweb-Edu-Chinese-V2.1) 项目 4-5 分集合等。

目录 `tidydata/` 记录了关联的数据清洗的脚本：
...
模型聪明与否取决于语料的规模和数据质量。因此该项目在数据清洗阶段，就对语料进行了深度分析，以确保数据的质量和数量。比如 BPE 分词之后的 PMI (点互信息)分析，通过排除了它们各自原本的热度后，识别它们绑在一起的纯粹程度。如果 PMI 很高，但语料数量又不太够，则需要补充相关的语料。
...
从第 1 和 6 条就可以看出，语料对于政治新闻类的内容覆盖有限，用这个语料训练的模型问特朗普相关问题，大概率会翻车。
...
该项目所需要的数据仅为 800M 的预训练 `data/raw/pretrain_clean.jsonl`，和 3000 条 SFT 环节的 `data/raw/qa_no_think.jsonl`。
...
为了相对全面评估模型的智力水平，本项目在以下方面进行了评估（1300 道通用领域题目可以在 `labs/files` 目录获得）：

- adversarial — 对抗鲁棒性, 100
- instruction — 指令遵循, 100
- knowledge — 知识记忆, 100
- language — 语言理解, 200
- machine_learning — 机器学习, 100
- math — 数学推理, 200
- memory — 记忆召回, 150
- pattern — 模式识别, 200
- reasoning — 逻辑推理, 100

language 下的 reading 子类：

```json
"question": "小许比小钱高，小徐比小钱矮。问：三人中谁最高？", 
"choices": ["无法确定", "小钱", "小徐", "小许"],
"answer": 4
```
...
```json
"question": "数列 6, 7, 6, 7, 6, 7，？中的下一个数是多少？",
"choices": ["8", "9", "6", "4"],
"answer": 3
```
knowledge 下的 general_knowledge 子类：

```json
"question": "下列哪部作品是鲁迅创作的？", 
"choices": ["《子夜》", "《茶馆》", "《骆驼祥子》", "《狂人日记》"], 
"answer": 4
```

pattern 下的 analogy 子类：

```json
"question": "铁之于金属，正如丝绸之于什么？", 
"choices": ["材料", "织物", "衣服", "纤维"], 
"answer": 2
```

以及我们期待有较好表现的 machine_learning 子类：

```json
"question": "机器学习中做特征选择时，可能用到的方法有", 
"choices": ["以上都有", "卡方", "期望交叉熵", "信息增益"], 
"answer": 1
```

在无梯度推理阶段，系统会将题干与四个选项分别进行序列拼接。模型会输出选项中每个真实 Token 对应的对数似然值。通过提取这些数值进行“求和并取平均”后，得分最高（即负值最接近 0）的选项，即为模型给出的最终答案。

在 `labs/08_benchmark.R` 脚本的实测中，测试五个模型上的最终准确率。

| model                      | Total | Correct | Accuracy |
|----------------------------|-------|---------|----------|
| Causal LM                  | 1300  | 384     | 29.54%   |
| JEPA VQ                    | 1300  | 363     | 27.92%   |
| Latent Residual            | 1300  | 368     | 28.31%   |
| MoE Causal LM              | 1300  | 396     | 30.46%   |
| 对比学习 (Contrastive)      | 1300  | 393     | 30.23%   |

在标准四选一题型中，随机猜测基线 (Random Baseline) 的数学期望为 25%，当然如果模型学习到了负向的推理或知识表达，也会无限接近 0 分。从五个模型的准确率来看，训练的模型已经初步建立知识表征与判别逻辑。

各个模型在 26 个子类上的准确率如下：

| 子维度 | Causal LM | JEPA | Latent Residual | MoE Causal LM | 对比学习 |
|--------|----------|---------|----------|-------------|--------------|
| 歧义消解 | 32.5% | 30.0% | 27.5% | 42.5% | 42.5% |
| 类比推理 | 26.0% | 24.0% | 28.0% | 18.0% | 24.0% |
| 算术运算 | 20.0% | 22.5% | 27.5% | 30.0% | 22.5% |
| 条件逻辑 | 2.0% | 2.0% | 2.0% | 6.0% | 20.0% |
| 约束满足 | 22.0% | 42.0% | 26.0% | 20.0% | 20.0% |
| 指代消解 | 40.0% | 30.0% | 27.5% | 55.0% | 32.5% |
| 干扰项识别 | 28.0% | 20.0% | 40.0% | 32.0% | 36.0% |
| 蕴含判断 | 40.0% | 35.0% | 30.0% | 30.0% | 32.5% |
| 事实回忆 | 22.0% | 18.0% | 18.0% | 24.0% | 18.0% |
| 格式遵循 | 16.0% | 30.0% | 14.0% | 20.0% | 10.0% |
| 常识知识 | 38.0% | 39.0% | 44.0% | 34.0% | 36.0% |
| 指令攻击 | 38.0% | 30.0% | 48.0% | 34.0% | 36.0% |
| 机器学习知识 | 35.0% | 29.0% | 30.0% | 34.0% | 28.0% |
| 多步推理 | 30.0% | 27.5% | 27.5% | 32.5% | 27.5% |
| 新颖规则 | 33.3% | 23.3% | 30.0% | 30.0% | 30.0% |
| 数字规律 | 20.0% | 30.0% | 24.0% | 22.0% | 20.0% |
| 排序顺序 | 20.0% | 16.0% | 22.0% | 24.0% | 32.0% |
| 阅读理解 | 40.0% | 37.5% | 32.5% | 37.5% | 32.5% |
| 关系回忆 | 26.0% | 24.0% | 22.0% | 32.0% | 18.0% |
| 反向回忆 | 24.0% | 16.0% | 20.0% | 16.0% | 18.0% |
| 选择筛选 | 26.0% | 28.0% | 22.0% | 32.0% | 42.0% |
| 语义关系 | 27.5% | 20.0% | 7.5% | 25.0% | 20.0% |
| 序列预测 | 25.0% | 35.0% | 27.5% | 27.5% | 20.0% |
| 三段论 | 40.0% | 28.0% | 38.0% | 42.0% | 60.0% |
| 符号规律 | 62.0% | 62.0% | 58.0% | 70.0% | 66.0% |
| 应用题 | 26.0% | 16.0% | 22.0% | 22.0% | 36.0% |
...
硬蒸就很暴力了，直接从开源模型通过批量化脚本生成语料，但硬蒸有个问题，Next Token Prediction 相当于截断了 Top1 个 token。而软蒸就比较聪明，可以把 TopK 的 token 对应的概率值全部表达出来，再通过 KL loss + CE loss 组合优化。

软蒸最好的办法是 tokenizer 一致，这样 token 的概率可以一一对应。到我们这个项目，小模型的词表大小为 2^14，远低于 Qwen/gemma 等模型的词表。这里应对的策略是做词表的对应归一，可参考作者博客。
...
本项目使用的是利用的依然还是 800 兆 JSONL 原始数据进行的蒸馏训练，该过程涉及教师模型推理（Qwen3.5-0.8B）。考虑到生态的完整性，使用 Python 将软标签数据以 arrow 的格式落盘，整个原始数据推理时间约为 5 小时。执行过程见：

```shell
python Distillation/01a_teacher_logits.py --limit 0        # 全量生成教师模型 Top 16 的概率值
...
- 知识记忆：45% → ~35%，-10pt，全图最大跌幅
- 对抗鲁棒性：34% → ~22%，-12pt，跌幅最大
- 记忆召回：28% → ~22%，-6pt
- 语言理解：35.5% → ~32%，-3.5pt

原因也很容易理解：蒸馏把学生的输出分布往教师在无关维基语料上的"平滑续写分布"方向拉，这种平滑化正好削弱了"精确匹配"的能力。结果就是上述同"精确匹配"相关的能力项的评分下降。

另外一个比较核心的原因就是该项目的模型参数规模太小，无法内化教师模型的暗知识。
...
接下来施加偏置，比如给绿名单增加 1 分，这时候分数变为了：开心（4.0）、高兴（3.9）、桌子（1.1）。模型采样时选择了“开心”，而且因为桌子的得分很低，即便是偏置后也无法同“开心、高兴”竞争，所以保证了模型不会胡言乱语（尤其是概率值已经低于阈值的区间，偏置不会触发）。
