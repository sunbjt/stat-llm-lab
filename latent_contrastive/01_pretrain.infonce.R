# =====================================================================
# Pure InfoNCE 预训练 (Train) — Raw Torch + Gradient Accumulation
# =====================================================================
...
RtomicBinDataset <- dataset(
  ...
  initialize = function(
    ...
    if (total_bytes %% bytes_per_token != 0) {
      stop(
        sprintf(
          "数据文件大小 (%d bytes) 不是 bytes_per_token=%d 的整数倍。",
          total_bytes,
          bytes_per_token
        )
      )
    ...
    raw_tokens <- readBin(
      ...
      what = "integer",
      n = total_tokens,
      size = bytes_per_token,
      signed = FALSE,
      endian = "little"
    )

    self$data_tensor <- torch_tensor(
      raw_tokens,
      dtype = torch_long()
    )
...
if (batches_per_epoch < 1) {
  stop(
    sprintf(
      "训练 batch 数为 0：dataset chunks=%d, batch_size=%d。",
...
# Embedding / norm / bias 不做 weight decay。
...
cat(sprintf(
  "Weight Decay (%.3f) 参数组: %d\n",
  WEIGHT_DECAY,
  length(decay_names)
))

cat(sprintf(
  "No Weight Decay 参数组: %d\n",
...
global_steps_per_epoch <- ceiling(
  batches_per_epoch / accum_steps
)
...
warmup_steps <- max(
  1,
  round(total_global_steps * warmup_pct)
)

decay_steps <- max(
  1,
  round(total_global_steps * decay_pct)
)

if (warmup_steps + decay_steps >= total_global_steps) {
  stop(
    "WSD schedule requires warmup_steps + decay_steps < total_global_steps."
  )
}

stable_steps <- total_global_steps -
  warmup_steps -
  decay_steps
...
log_file <- paste0(
  "checkpoints/contrastive_loss_",
  format(Sys.time(), "%d%H%M"),
  ".csv"
)

cat(
  "epoch,iter,loss,contrastive,lr,global_step\n",
  file = log_file
)

scaler <- if (ENV_USE_AMP) {
...
# First optimizer update uses schedule step 1.
...
for (epoch in 1:TRAIN_EPOCHS) {
  ...
  coro::loop(for (batch in train_dl) {
    ...
    current_accum_steps <- if (
      remainder != 0 &&
      batch_idx > batches_per_epoch - remainder
    ) {
      remainder
    } else {
      accum_steps
    ...
    if (should_step) {
      ...
      # Update LR for the next optimizer step.
      current_lr <- BASE_LR *
        wsd_multiplier(
          min(global_step + 1, total_global_steps)
        )
