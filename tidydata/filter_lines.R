...
pattern_trad <- "[漸這們會說過對還個為與從時進學實發動網機體點廠廣車頁見長門風飛馬鳥麥東魚愛]"
...
while (length(lines <- readLines(con_in, n = chunk_size, warn = FALSE)) > 0) {
  ...
  hit_url <- grepl("根据以下|意识形态|人工智能在医疗|更好地理解|例如，可以使用|人工智能可以通过|他们的对话|以下是|网址|来源|script|两个维护|全市|总书记|同志|市委|例子|防控|可以根据学生的学习", texts)
